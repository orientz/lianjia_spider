...
def getURL(url, tries_num=50, sleep_time=0, time_out=10):
    ...
    proxies ={ "http": "182.61.58.202:8080", "https": "http://182.61.58.202:8080" }#替换为可用代理IP
...
def getXiaoquList(fang_url):
    ...
    for fang in soup.select('.xiaoquListItem'):
        if (len(fang) > 0):
            try:
                result['xiaoqu_key'] = fang.select('.houseInfo')[0].select('a')[0].text.strip()
                result['xiaoqu_name'] =  fang.select('.title')[0].text.strip()
                result['xiaoqu_url'] = fang.select('.title')[0].a['href'].strip()
                result['quyu'] = fang.select('.positionInfo')[0].select('a')[0].text.strip()
                result['bankuai'] =  fang.select('.bizcircle')[0].text.strip()
                result['price'] = fang.select('.totalPrice')[0].span.text.strip()

                # result['xiaoqu_key'] = '1'
                #result['xiaoqu_name'] = '1'
                #result['xiaoqu_url'] = '1'
                #result['quyu'] ='1'
                # result['bankuai'] = '1'
                # result['price'] ='1'
...
def getSubRegions(fang_url, region):
    base_url = 'http://sh.lianjia.com'
    res = getURL(base_url + region['link'])
    print fang_url
    print region['link']
    res.encoding = 'utf-8'
    soup = BeautifulSoup(res.text, 'html.parser')
    ...
    distemp2 = gio_plate.find_all('div')
    #gio_plate = soup.find('div', class_="option-list sub-option-list gio_plate")
    try:
        for link in distemp2[1].find_all('a'):
            district = {}
            district['link']=link.get('href')
            templink =link.get('href')
            arrayTemp = templink.split('/')
            district['code'] = arrayTemp[2]
...
def getRegions(fang_url, region):
    ...
    distemp = gio_district.find_all('div')
    for link in distemp[0].find_all('a'):
        district = {}
        district['link']=link.get('href')
...
def getTransMain():
    regions = getRegions('http://sh.lianjia.com/ershoufang/', 'pudong')
    regions.reverse()
    #start_page = 1
    #end_page =10
    #sleep_time = 0.5
    ...
    while regions:
        ...
        subRegions.reverse()
        while subRegions:  # and  region in  ['pudong','minhang','baoshan','xuhui','putuo','yangpu','changning','songjiang','jiading','huangpu','jingan','zhabei','hongkou','qingpu','fengxian','jinshan','chongming','shanghaizhoubian']:
            try:
                subRegion = subRegions.pop()
                print getCurrentTime(), region['name'], ':', subRegion['name'], 'Scrapy Starting.....'
                # time.sleep(sleep_time)
                for i in range(start_page, end_page):
                    chengjiao_url = 'http://sh.lianjia.com/chengjiao/' + subRegion['code'] + '/pg' + str(i)
...
def main():
    ...
    mySQL = MySQL()
    mySQL._init_('182.61.58.202', 'root', 'Zyy_1993215', 'lianjia_ershoufang')
